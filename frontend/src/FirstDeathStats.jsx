...
const getAbilityName = (heroName, abilityRaw) => {
    ...
    // 💡 스킬 이름 뒤에 단축키(Shift, E, Q) 예쁘게 붙여주기
            ...
            matches.forEach(match => {
                ...
                matchEvents.forEach(ev => {
                    ...
                    if (!curF || ev.timestamp > curF.fixedEndTime) {
                        ...
                        curF = { startTime: ev.timestamp, fixedEndTime: ev.timestamp + 20, first_pick_event: null };
                    }
                    if (ev.event_type === 'kill' && !curF.first_pick_event) {
                ...
                fights.forEach(f => {
                    if (f.first_pick_event) {
                        ...
                        const name = normalizeName(ev.target_name);
                        if (pMap[name]) {
                            pMap[name].firstDeaths += 1;
                            pMap[name].deathLogs.push({
                                killerName: normalizeName(ev.player_name),
                                killerHero: ev.player_hero,
                                skill: getAbilityName(ev.player_hero, ev.ability),
                                detailedSkill: `${normalizeName(ev.player_name)}|${ev.player_hero}|${getAbilityName(ev.player_hero, ev.ability)}`
                            });
                        }
                    }
                });
    ...
    const sortedData = useMemo(() => {
        ...
        const sorted = [...filtered].sort((a, b) => {
            ...
            if (sortConfig.key === 'rate') {
                valA = a.totalFights > 0 ? (a.firstDeaths / a.totalFights) : 0;
                valB = b.totalFights > 0 ? (b.firstDeaths / b.totalFights) : 0;
            }
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: isMobile ? 640 : undefined }}>
                    <thead style={{ background: theme.surfaceHighlight }}>
                        <tr>
                            ...
                            <th style={{ padding: '16px', textAlign: 'right', fontSize: '13px', color: theme.textSub, cursor:'pointer' }} onClick={() => handleSort('firstDeaths')}>{t.fdColCount} <ArrowUpDown size={12} style={{display:'inline'}}/></th>
                            <th style={{ padding: '16px', textAlign: 'right', fontSize: '13px', color: DANGER_COLOR, cursor:'pointer', fontWeight:'900' }} onClick={() => handleSort('rate')}>{t.fdColRate} <ArrowUpDown size={12} style={{display:'inline'}}/></th>
                            ...
                            return (
                                <React.Fragment key={p.name}>
                                    <tr onClick={() => toggleExpand(p.name)} style={{ background: isExpanded ? theme.surfaceHighlight : rowBg, borderBottom: isExpanded ? 'none' : `1px solid ${theme.border}40`, transition: 'background 0.2s', cursor: 'pointer', opacity: lowSample ? 0.5 : 1 }} onMouseOver={e=>e.currentTarget.style.background=theme.surfaceHighlight} onMouseOut={e=>e.currentTarget.style.background=isExpanded ? theme.surfaceHighlight : rowBg}>
                                        ...
                                        <td style={{ padding: '16px', textAlign: 'right', fontWeight: 'bold' }}>{p.firstDeaths}{t.timesUnit}</td>
                                        <td style={{ padding: '16px', textAlign: 'right' }}>
                                            <div style={{ position: 'relative', width: '100%', height: '24px', background: theme.surface, borderRadius: '4px', overflow: 'hidden' }}>
                                                <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: `${Math.min(rate * 2, 100)}%`, background: isDanger ? '#ef444480' : '#ef444440' }} />
                                                <span style={{ position: 'relative', zIndex: 1, paddingRight: '8px', fontWeight: '900', fontSize: '13px', color: isDanger ? DANGER_COLOR : theme.text }}>{rate.toFixed(1)}%</span>
                                            </div>
                                        </td>
                                                ...
                                                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: isMobile ? '12px' : '20px', background: theme.bg, padding: isMobile ? '14px' : '20px', borderRadius: '12px', border: `1px solid ${theme.border}40` }}>
                                                    <div>
                                                        ...
                                                        {getTopN(p.deathLogs, 'killerName', 5).map((target, i) => {
                                                            const pct = p.firstDeaths > 0 ? (target.count / p.firstDeaths) * 100 : 0;
                                                            return (
                                                                <div key={i} style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', padding: '6px 8px', marginBottom: '4px', borderRadius: '4px', background: 'rgba(255,255,255,0.03)', overflow: 'hidden' }}>
                                                                    ...
                                                                    <span style={{ position: 'relative', zIndex: 1, fontSize: '13px', fontWeight: 'bold', color: DANGER_COLOR }}>{target.count}{t.timesUnit} <span style={{fontSize:'10px', color:theme.textSub, fontWeight:'normal'}}>({pct.toFixed(0)}%)</span></span>
                                                                </div>
                                                            )
                                                        })}
                                                    </div>
                                                    <div>
                                                        <h4 style={{ fontSize: '13px', color: theme.textSub, marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}><Shield size={14} color={DANGER_COLOR}/> {t.fdWatchHeroes}</h4>
                                                        {getTopN(p.deathLogs, 'killerHero', 5).map((hero, i) => {
                                                            const pct = p.firstDeaths > 0 ? (hero.count / p.firstDeaths) * 100 : 0;
                                                            return (
                                                    ...
                                                    <div>
                                                        <h4 style={{ fontSize: '13px', color: theme.textSub, marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}><Sword size={14} color={DANGER_COLOR}/> {t.fdDeadlySkill}</h4>
                                                        {getTopN(p.deathLogs, 'detailedSkill', 5).map((log, i) => {
                                                            // 💡 플레이어 이름 제거, 영웅과 스킬만 표시!
                                                            const [kName, kHero, skill] = log.name.split('|');
                                                            const pct = p.firstDeaths > 0 ? (log.count / p.firstDeaths) * 100 : 0;
                                                            return (
                                                                <div key={i} style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 8px', marginBottom: '4px', borderRadius: '4px', background: 'rgba(255,255,255,0.03)', overflow: 'hidden' }}>
                                                                    <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: `${pct}%`, background: `${DANGER_COLOR}20`, zIndex: 0 }} />
                                                                    <div style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                                        <img src={getHeroImageSrc(kHero)} style={{ width: 16, height: 16, borderRadius: 2 }} onError={e=>e.currentTarget.style.display='none'}/>
                                                                        <span style={{ fontSize: '13px', fontWeight: 'bold' }}>{skill}</span>
                                                                    </div>
